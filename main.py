...
class Opp:
    ...
    def check_action(self, r, window, players):
        if self.position == 'sb' and r == 0:
            ...
            coord = self.action_info[self.seat]
            count = 0
            while True:
                rect = win32gui.GetWindowRect(window)
                screen = ImageGrab.grab((rect[0], rect[1], rect[2], rect[3]))
                ...
                count += 1
                actions = [action for action in self.action_string]
                accuracy = [np.sum(value == img) / img.size for key, value in self.action_string.items()]
                # print(accuracy)
    ...
    def call(self, window, for_call):
        ...
        self.size_bet = for_call + self.size_bet
        print(self.position + ' колл ' + str(self.size_bet))

    def bet(self, window, bb):
        self.action[self.street].append('R')
        size0 = self.stack
        self.update_stack(window)
        size1 = self.stack
        self.size_bet = size0 - size1
        print(self.position + ' повысил на ' + str(self.size_bet))
                    ...
                    if 'R' in actions:
                        ...
                        if actions.count('R') == 1:
                            if self.preflop_card() in self.cards_3bet[raiser.position][1]:
                                self.bet(window, int(raiser.size_bet * 3))
                            elif self.preflop_card() in self.cards_cold_call[raiser.position]:
                                for_call = max([p.size_bet for p in players]) - self.size_bet
                                self.call(window, for_call)
                            else:
                                self.fold(window)
            ...
            while True:
                ...
                if r == 0:
                    for player in self.players:
                        ...
                        player.check_action(r, self.window, players_playing)
                    r += 1
                elif r == 1:
                    for player in self.players:
                        self.pot = sum([p.size_bet for p in self.players])
                        if not hero.in_play:
                            return
                        max_size_bet = max([p.size_bet for p in self.players if p.in_play])
                        print(max_size_bet)
                        players_playing = [p for p in self.players
                                           if p.in_play if p != player]
                        if not players_playing:
                            if hero.in_play:
                                print('Вы выиграли ' + str(self.pot - hero.size_bet))
                                return
                        if player.in_play:
